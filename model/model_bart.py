...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    For example: 
        input_ids: [[0,  7424, 634, 34369,   718,    2,     1,     1,      1]]
        after shifting: [[2, 0,  7424, 634, 34369,   718,    2,     1,     1]]
    """
...
def _make_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0):
    ...
    mask = torch.full((tgt_len, tgt_len), float("-inf"))# a matrix [tgt, tgt] dimension with all values are "-inf"
    mask_cond = torch.arange(mask.size(-1))# a vector [tgt] dimension [0, 1, ..., tgt-1]
    mask.masked_fill_(mask_cond < (mask_cond + 1).view(mask.size(-1), 1), 0)# matrix [tgt, tgt]
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        embed_pos = self.embed_positions(input_shape, position_ids)
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        # embed positions
        positions = self.embed_positions(input_shape, position_ids, past_key_values_length)
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        #This ```forward``` is provided a new argument: ```position_ids```
        #```position_ids``` is set to the encoder and it is None in the decoder
