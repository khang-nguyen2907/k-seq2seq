...
from model.model_gpt2 import GPT2LMHeadModel
from model_saver import *
from seed import *

from model.model_bart import *
from model.model_encoder_decoder import EncoderDecoderModel
from transformers import AutoConfig, AutoModel, AutoTokenizer, GPT2Tokenizer, AutoConfig, EncoderDecoderConfig
from transformers import AdamW, get_scheduler

# DECODER_SPECIAL_TOKENS  = {"bos_token": "<|BOS|>",
#                    "eos_token": "<|EOS|>",
#                    "unk_token": "<|UNK|>",                    
#                    "pad_token": "<|PAD|>",
#                    "sep_token": "<|SEP|>"} 
...
class Medical_Datset(Dataset): 
    def __init__(self, dataset_path, encoder_tokenizer, decoder_tokenizer,args, qna = False, ij_kg = False): 
        ...
        self.decoder_vocab_file = decoder_tokenizer.vocab
        self.sentences = self.load_sentences()
        self.columns = self.load_columns()
        self.decoder_tokenizer = decoder_tokenizer
        self.encoder_tokenizer = encoder_tokenizer
        #it indicates that this dataset is loaded for QuestionAnswering task or Generation task
        self.qna = qna
        #it indicates that input of decoder has knowledge graph information or not
        self.ij_kg = ij_kg
...
def main():
    ...
    #####################################################################################################
    #LOADING PREVIOUS INFORMATION FOR KEEPING TRAINING (IF POSSIBLE)
    start_epoch = 1
    last_epoch = 0
    best_result = 9999.0
    ...
    def evaluate(args, is_test):
        if is_test: 
            eval_dataset = Medical_Datset(dataset_path= args.test_path, encoder_tokenizer=roberta_tokenizer, decoder_tokenizer= gpt2_tokenizer, args= args, qna= True, ij_kg = True)
            eval_loader = DataLoader(dataset = eval_dataset, batch_size=args.batch_size)
        else: 
            eval_dataset = Medical_Datset(dataset_path= args.dev_path, encoder_tokenizer=roberta_tokenizer, decoder_tokenizer= gpt2_tokenizer, args= args, qna= True, ij_kg = True)
            eval_loader = DataLoader(dataset = eval_dataset, batch_size = args.batch_size)
    ...
    train_dataset = Medical_Datset(dataset_path= args.train_path,encoder_tokenizer=roberta_tokenizer, decoder_tokenizer= gpt2_tokenizer, args = args, qna = True, ij_kg = True)
    ...
    for epoch in range(start_epoch, last_epoch + args.epochs_num + 1):
        ...
        for batch_idx, inputs in loop: 
            ...
            decoder_visible_matrix = inputs[5]
            decoder_position_ids = inputs[6]
            decoder_seg_mask = inputs[7]
            labels_ids = inputs[8]
            ...
            outputs = model(
                ...
                decoder_attention_mask=decoder_visible_matrix,
